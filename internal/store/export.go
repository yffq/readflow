...
func (s *Store) ListArticleSummariesSince(updatedAfter time.Time, limit, offset int) ([]model.ArticleExport, int, error) {
	var count int
	countSQL := "SELECT COUNT(*) FROM articles WHERE updated_at > ?"
...
func (s *Store) ListArticleSummaryPageSince(updatedAfter time.Time, limit, offset int) ([]model.ArticleExport, error) {
	querySQL := `SELECT id, title, url, author, site_name, word_count, source, extraction_failed, status, created_at, updated_at
		FROM articles WHERE updated_at > ? ORDER BY updated_at DESC LIMIT ? OFFSET ?`
